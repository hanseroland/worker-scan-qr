...
export class LoginUseCase {
      ...
      async execute(email:string,password:string): Promise<{ user: SafeUserDTO, accessToken: string, refreshToken: string }>{
        ...
        // 3. Comparer password
        const isPasswordValid = await this.passwordService.compare(password, existingUser.password);
        if(!isPasswordValid) throw new AuthError('Invalid password');

        // 4. Générer accessToken 
        const accessToken = this.tokenService.generateAccessToken(existingUser);

        // 5. Générer refreshToken
        const refreshToken = this.tokenService.generateRefreshToken(existingUser);
